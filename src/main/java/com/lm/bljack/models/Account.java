...
public class Account {
	...
	public String setPlayOutcome(int pointsPlayer, int pointsDealer, GameStatus statusP, GameStatus statusD,
			...
			if      (pointsPlayer > 21)                                                       return "bust";
			...
			else if (pointsPlayer < 22 && pointsDealer > 21)                                  return "21u_bust";	
	...
	/* Calculate payoffs given bet amount & play outcome 
	   ...
	   */
	public int setAccountAfterPayoff(String type, int bet, boolean EvenMoney) {
		
		double evenMoneyPayoff = 0;
		if (EvenMoney) evenMoneyPayoff = 2* 0.5 * bet;
		...
		else if(type.equals("blj_blj")) 		return     account + (int)evenMoneyPayoff;
    ...
    public String generateWinLooseMsg(String betOutcome, int bet){ 
		...
		if  (betOutcome.equals("bust")) 		  message = "You loose $" + bet;
		...
		else if (betOutcome.equals("21u_more"))   message = "You loose $" + bet;		
